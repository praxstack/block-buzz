...
1. **Scan at sink first.** The repo copy leaves the private store, so scan the
   ...
   `~/.cursor/skills/gstack/bin/gstack-redact --from-file <tmp>`; exit 3
...
```bash
~/.cursor/skills/gstack/bin/gstack-config get founder_resources 2>/dev/null || echo "true"
```
...
`~/.cursor/skills/gstack/bin/gstack-config set founder_resources false`, then
