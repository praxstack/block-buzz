...
```bash
[ -f ~/.cursor/skills/gstack/bin/gstack-code-intelligence ] \
  && bun ~/.cursor/skills/gstack/bin/gstack-code-intelligence select gbrain 2>/dev/null \
  || true
```
...
bulk via `bun run ~/.cursor/skills/gstack/bin/gstack-memory-ingest.ts --bulk --quiet`. Set
...
hook runs `bun run ~/.cursor/skills/gstack/bin/gstack-gbrain-sync.ts --incremental --quiet` on every skill
