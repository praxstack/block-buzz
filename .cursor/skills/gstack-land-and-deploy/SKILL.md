...
```bash
~/.cursor/skills/gstack/bin/gstack-evidence check --label tests --expect-cmd '<the project test command>' --max-age 24 --allow-paths CHANGELOG.md,VERSION,package.json,agents-digest/gstack-AGENTS.md
```
...
```bash
~/.cursor/skills/gstack/bin/gstack-evidence run --label tests -- 'bun test 2>&1'
```
...
```bash
~/.cursor/skills/gstack/bin/gstack-issue-guard pr-body
```
