...
```bash
~/.cursor/skills/gstack/bin/gstack-config get pair_agent 2>/dev/null || echo "unset"
```
...
Options: A) Enable — run `~/.cursor/skills/gstack/bin/gstack-config set pair_agent on`, confirm it reads back `on`, and continue. B) No — stop here; local pairing (option A above) still works.
